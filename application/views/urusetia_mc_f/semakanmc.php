            ...
            <div class="row">
                ...
                <div class="col-lg-12 m-b30">
                    <div class="widget-box">
                      <div class="m-4 pt-3">
                        <a href="<?=base_url('/urusetia_mc/semak_mc/').$this->encryption->encrypt('all');?>" class="btn green" style="color:#fff;">All</a>
                        <a href="<?=base_url('/urusetia_mc/semak_mc/').$this->encryption->encrypt('waiting');?>" class="btn blue" style="color:#fff;">Waiting for Verification</a>
                        <a href="<?=base_url('/urusetia_mc/semak_mc/').$this->encryption->encrypt('unpaid');?>" class="btn red" style="color:#fff;">Unpaid</a>
                      </div>
                                ...
                                ++$num; ?>
                                    <tr>
                                        ...
                                        <td style="text-align:left; vertical-align:middle;">
                                          <ol class="ml-4 mb-0">
                                            <?php
                                            $invoicedetails = $this->urusetiamc_m->get_param_kursus_by_invoice($row->user_id,$row->noinvoice);
                                            if($invoicedetails) {
                                              $i = 0;
                                              $totalstatusenrol = 0;
                                              foreach ($invoicedetails as $value) {
                                                $i += 1;
                                                $mdl_courselist = $this->urusetiamc_m->get_course_detail_mcodlukm($value->kursusid);
                                                $dataenrol = array(
                                                  'order_mc' => $row->id,
                                                  'nomatrik' => $row->nomatrik_mc,
                                                ...
                                                if($enrol){
                                                  $liststatus2 = $this->urusetiamc_m->get_liststatus($enrol[0]->status_enrol);
                                                  if($liststatus2) {
                                        ...
                                        <td style="text-align:left; vertical-align:middle;">
                                          <?php
                                            $liststatus = $this->urusetiamc_m->get_liststatus($row->status_app);
                                            echo "Application: ";
                                        ...
                                        //    $liststatus1 = $this->urusetiamc_m->get_liststatus($row->status_urus);
                                        //    if($liststatus1) {
                                        ...
                                        <td align="center" style="vertical-align:middle;">
                                            <?php $parameter = $row->noinvoice; //dbug($parameter);die();?>
                                              <!--<a name="detail_pemohon" href="<?php echo base_url('/urusetia_mc/email_compose/'.$this->encryption->encrypt($parameter)); ?>" class="btn button-sm blue radius-xl mb-2" title="Email"><i class="fa fa-envelope-o"></i></a>
                                              -->
                                            <?php if($liststatus[0]->status_id == '4' || $liststatus[0]->status_id == '6' || $row->status_urus == '7') { ?>
                                              <a name="detail_pemohon" href="<?php echo base_url('/urusetia_mc/detail_pemohon/'.$this->encryption->encrypt($parameter)); ?>" class="btn button-sm blue" title="Action"><i class="fa fa-pencil"></i></a>
                                            <?php } else if($liststatus[0]->status_id != '3' || $row->status_urus == '7') { ?>
                                              <a name="detail_pemohon" href="<?php echo base_url('/urusetia_mc/detail_pemohon/'.$this->encryption->encrypt($parameter)); ?>" class="btn button-sm yellow" title="Action"><i class="fa fa-pencil"></i></a>
                                            <?php } ?>

                                              <!--<a name="detail_pemohon" href="<?php echo base_url('/urusetia_mc/payment_pemohon/'.$this->encryption->encrypt($parameter)); ?>" class="btn bg-info" title="Action">Payment Detail</a>-->
