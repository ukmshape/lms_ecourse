...
// echo 'coustom session';
     ...
     */
     ...
     {
        $this->_ci->load->config('session',TRUE);
        $sess_config = $this->_ci->config->item('session');
        $sess_namespace = $sess_config['sess_namespace'];

        if (empty($sess_namespace)){
            ...
            //setkan namespace
            $sess_namespace = $kodsistem;

            //update config
            $sess_config['sess_namespace'] = $sess_namespace;
            $this->_ci->config->set_item('session', $sess_config );



            if (!$sess_namespace = $this->_ci->config->item('kodsistem')){
                $this->_ci->load->config('application',TRUE);
                $sess_namespace = $this->_ci->config->item('kodsistem','application');
            }
        }
        ...
        foreach ($prefs as $key) {
            $this->_config[$key] = $this->_ci->config->item($key);
        }
        ...
        if (!empty($last_activity) && (($last_activity + $expire) < $now or $last_activity > $now)) {
            ...
            && $user_agent !== trim(substr($this->_ci->input->user_agent(), 0, 50))) {
    ...
    public function sess_create()
    {
        ...
        if ($this->_config['sess_match_ip'] === true) {
            ...
            $_SESSION[$this->sess_namespace]['ip_address'] = $this->_ci->input->ip_address();
        }

        if ($this->_config['sess_match_useragent'] === true) {
            // Store user agent string
            $_SESSION[$this->sess_namespace]['user_agent'] = trim(substr($this->_ci->input->user_agent(), 0, 50));
        }
        ...
        if (is_null($key)){
            foreach($this->all_userdata() as $key => $val){
                if(strpos($key,'flash:old:') > -1){ // key is flashdata
                    ...
                    $this->_keep_flashdata($item);
                }
            }
        }elseif (is_string($key)){
            $this->_keep_flashdata($key);
        }elseif (is_array($key)){
            foreach ($key as $value) {
                $this->_keep_flashdata($value);
            }
        }
       
    }

    private function _keep_flashdata($key)
    {
        // 'old' flashdata gets removed.  Here we mark all
        // flashdata as 'new' to preserve it from _flashdata_sweep()
   ...
   /**
    ...
    {
        //semak dalam flashdata new dulu
        $flashdata_key = $this->flashdata_key.':new:'.$key;
        if ($flashdata_data = $this->userdata($flashdata_key))
            return $flashdata_data;
